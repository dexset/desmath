...
long combinationCount( long n, long k ) pure nothrow
in
{
    assert( k > 0 );
    assert( n >= 0 );
...
unittest
{
    ...
    foreach( k; 1 .. 10 )
        assertEq( combinationCount(10,k), comb2(10,k),
                  format( "equal test fails with k==%d, %%s != %%s", k ) );
}

/// equals to fact(n) / fact(n-k)
long partialPermutationCount( long n, long k ) pure nothrow
in
{
    assert( k > 0 );
    assert( n >= k );
...
unittest
{
    ...
    foreach( k; 1 .. 10 )
        assertEq( partialPermutationCount(10,k), perm2(10,k),
                  format( "equal test fails with k==%d, %%s != %%s", k ) );
}
