module des.math.util.mathstruct;
...
unittest
{
    ...
    assertEq( p1 + p2, Val(3,5) );
    assertEq( p2 - p1, Val(1,1) );
    assertEq( p1 * 3, Val(3,6) );
    assertEq( p1 / 2, Val(0.5,1) );
    ...
    assertEq( c1 + c2, Comp("ololo", 15, 1.3) );
}
...
unittest
{
    ...
    assertEq( a + a, Point( Vec(2,4), Vec(4,6), "hello", 6 ) );
    assertEq( a * 2, Point( Vec(2,4), Vec(4,6), "hello", 6 ) );
}
...
private
{
    ...
    unittest
    {
        assertEq( opSelf( "pnt rot".split, "+", "vv" ), "pnt + vv.pnt, rot + vv.rot" );
        assertEq( opSelf( "a b c".split, "*", "x" ), "a * x.a, b * x.b, c * x.c" );
    }
    ...
    unittest
    {
        assertEq( opEach( "pnt rot".split, "+", "vv" ), "pnt + vv, rot + vv" );
        assertEq( opEach( "a b c".split, "*", "x" ), "a * x, b * x, c * x" );
    }
    ...
    unittest
    {
        assertEq( argName( "pos" ), "arg_pos" );
        assertEq( argName( "Pos" ), "arg_Pos" );
        assertEq( argName( "Pos.x" ), "arg_Pos_x" );
        assertEq( argName( "P.s.x" ), "arg_P_s_x" );
    }
